...
type User struct {
	...
	LastDeposit            uint64    `gorm:"type:bigint;not null"`
...
func (ui *UserInfoRepo) UpdateUserNewTwoNewTwoTwo(ctx context.Context, userId int64, amount uint64, last uint64, coinType string) error {
	if "USDT" == coinType {
		res := ui.data.DB(ctx).Table("user").Where("id=?", userId).
			Updates(map[string]interface{}{"last_deposit": last, "amount": gorm.Expr("amount + ?", amount)})
		if res.Error != nil {
...
func (ui *UserInfoRepo) UpdateUserRewardRecommend3(ctx context.Context, userId, recommendUserId int64, userAddress string, amountUsdtAll float64, amountUsdt float64, amountNana float64, amountUsdtOrigin float64, recommendTwo, stop bool) (int64, error) {
	var err error
...
func (ui *UserInfoRepo) UpdateUserRewardRecommend2(ctx context.Context, userId, recommendUserId int64, userAddress string, amountUsdt float64) (int64, error) {
	var err error
...
// GetUserRecommendByUserId .
func (ui *UserInfoRepo) GetUserRecommendByUserId(ctx context.Context, userId int64) (*biz.UserRecommend, error) {
	var userRecommend UserRecommend
	if err := ui.data.db.Where(&UserRecommend{UserId: userId}).Table("user_recommend").First(&userRecommend).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, errors.NotFound("USER_RECOMMEND_NOT_FOUND", "user recommend not found")
		}
...
// UpdateUserNewSuper .
func (ui *UserInfoRepo) UpdateUserNewSuper(ctx context.Context, userId int64, amount, num int64) error {

	res2 := ui.data.DB(ctx).Table("user").Where("id=?", userId).
		Updates(map[string]interface{}{"amount_biw": amount, "last_deposit": num})
	if res2.Error != nil {
		return errors.New(500, "UPDATE_USER_ERROR", "用户信息修改失败")
	}
...
// GetAllUsers .
func (ui *UserInfoRepo) GetAllUsers(ctx context.Context) ([]*biz.User, error) {
	var users []*User
...
// GetUserRecommends .
func (ui *UserInfoRepo) GetUserRecommends(ctx context.Context) ([]*biz.UserRecommend, error) {
	var userRecommends []*UserRecommend
	res := make([]*biz.UserRecommend, 0)
	if err := ui.data.db.Table("user_recommend").Find(&userRecommends).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return res, errors.NotFound("USER_RECOMMEND_NOT_FOUND", "user recommend not found")
		}
...
// GetUserAddress .
func (ui *UserInfoRepo) GetUserAddress(ctx context.Context, userId uint64) ([]*biz.UserAddress, error) {
	var userAddress []*UserAddress
	res := make([]*biz.UserAddress, 0)
	if err := ui.data.db.Where("user_id=?", userId).Table("user_address").Find(&userAddress).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (ui *UserInfoRepo) UpdateUserRewardArea(ctx context.Context, userId, i, level int64, userAddress string, amountUsdtAll float64, amountUsdt float64, amountNana float64, amountUsdtOrigin float64, tmpLevel, stop bool) (int64, error) {
	var err error
...
// UpdateUserRewardAll .
func (ui *UserInfoRepo) UpdateUserRewardAll(ctx context.Context, userId int64, amountUsdtAll float64, amountUsdt float64, amountNana float64, amountUsdtOrigin float64, stop bool) (int64, error) {
	var err error
	...
	if err = ui.data.DB(ctx).Table("user_balance").
		...
		Updates(map[string]interface{}{
			"balance_usdt_float": gorm.Expr("balance_usdt_float + ?", amountUsdt),
			//"balance_raw_float":  gorm.Expr("balance_raw_float + ?", amountNana),
			"all_float": gorm.Expr("all_float + ?", amountUsdtAll),
		}).Error; nil != err {
		return 0, errors.NotFound("user balance err", "user balance not found")
	}
	...
	//reward.AmountNewTwo = amountNana
	reward.BalanceRecordId = userBalanceRecode.ID
	reward.Type = "system_reward_all_daily" // 本次分红的行为类型
	reward.Reason = "all"                   // 给我分红的理由
	err = ui.data.DB(ctx).Table("reward").Create(&reward).Error
	if err != nil {
		return 0, err
	}
...
func (ui *UserInfoRepo) UpdateUserNewTwoNewThree(ctx context.Context, userId int64, amount uint64, last int64, coinType string) error {
	if "USDT" == coinType {
		res := ui.data.DB(ctx).Table("user").Where("id=?", userId).
			Updates(map[string]interface{}{"amount": amount})
		if res.Error != nil {
			return errors.New(500, "UPDATE_USER_ERROR", "用户信息修改失败")
		}
	} else {
...
func (ui *UserInfoRepo) UpdateUserLast(ctx context.Context, userId int64, coinType string) error {
	if "USDT" == coinType {
		res := ui.data.DB(ctx).Table("user").Where("id=? and last_deposit>=?", userId, 0).
			Updates(map[string]interface{}{"last_deposit": 0})
		if res.Error != nil {
			return errors.New(500, "UPDATE_USER_ERROR", "用户信息修改失败")
		}
...
func (ub UserBalanceRepo) GetUserBalanceUsdtTotal(ctx context.Context) (float64, error) {
	var total UserBalanceTotalFloat
	if err := ub.data.db.Table("user_balance").Select("sum(balance_usdt_float) as total").Take(&total).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return total.Total, errors.NotFound("USER_BALANCE_NOT_FOUND", "user balance not found")
		}
...
func (ub UserBalanceRepo) GetUserBalanceDHBTotal(ctx context.Context) (float64, error) {
	var total UserBalanceTotalFloat
	if err := ub.data.db.Table("user_balance").Select("sum(balance_raw_float) as total").Take(&total).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return total.Total, errors.NotFound("USER_BALANCE_NOT_FOUND", "user balance not found")
		}
...
func (ub UserBalanceRepo) GetUserBalanceRecordCountTotalToday(ctx context.Context) (int64, error) {
	...
	if err := ub.data.db.Table("user_balance_record").
		...
		Group("user_id").
		Where("created_at>=?", todayStart).Where("created_at<?", todayEnd).
		Select("count(id) as total,user_id").Take(&total).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return total.Total, errors.NotFound("USER_BALANCE_RECORD_NOT_FOUND", "user balance not found")
		}
...
func (ub UserBalanceRepo) GetUserWithdrawDhbTotalToday(ctx context.Context) (float64, error) {
	var total UserBalanceTotalFloat
	now := time.Now().UTC()
	var startDate time.Time
	var endDate time.Time
...
func (ub UserBalanceRepo) GetUserWithdrawDhbTotal(ctx context.Context) (float64, error) {
	var total UserBalanceTotalFloat
	if err := ub.data.db.Table("user_balance_record").
		Where("type=?", "withdraw").
		Where("coin_type=?", "NANA").
